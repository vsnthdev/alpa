...
export const Header = (): ReactElement => {
    ...
    return <header className="bg-white border-b-2 border-slate-200 py-4">
        <div className="container mx-auto flex justify-between items-center px-8">
            ...
            {/* logout button */}
            {auth.isLoggedIn
            ? <button className='font-semibold py-2 px-4 border border-transparent text-sm rounded-md transition-colors text-white bg-secondary hover:bg-secondary-hover outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-800' onClick={() => logout({ auth, navigate, dispatch })}>Logout</button>
            : ""}
        </div>
    </header>
}
